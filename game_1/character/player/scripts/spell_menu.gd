...
var base_color: Color = Color("ab3e37ff")
var highlight_color: Color = Color("c04941ff")
var line_color: Color = Color("e7756aff")

var spells: Array[String] = ["ball", "explosion", "curse"]
...
func _draw() -> void:
	...
	# draw spells on circle
	for i in range(len(spells)):
		# TAU = 2pi
		var start_rad: float = TAU * i / len(spells)
		var end_rad: float = TAU * (i + 1) / len(spells)
	...
	for i in range(len(spells)):
		var rad: float = TAU * i / len(spells)
